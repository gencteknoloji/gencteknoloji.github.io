...
const SIDEBAR_GROUPS: { title: string; items: SidebarNavItem[]; accent?: 'turkcell' }[] = [
  ...
  },
  {
    title: 'Analiz & Rapor',
    items: [
      { key: 'dashboard', label: 'Analiz', icon: LayoutDashboard },
      { key: 'history', label: 'Satış Geçmişi', icon: ArrowRightLeft },
      { key: 'reports', label: 'Satış Raporları', icon: TrendingUp },
    ],
  },
  {
    title: 'Stok',
...
export default function DashboardHome() {
  ...
  const loadAllData = async (silent = false) => {
    try {
      ...
      const dashData = await dbService.getDashboard();
      const tcellData = await dbService.getTurkcellPremiums();
      const expensesData = await dbService.getExpenses();
      const tcellStockData = await dbService.getTurkcellStock();
      const closingsData = await dbService.getDailyClosings();

      setCustomers(custData);
      setProducts(prodData);
      setSales(salesData);
      setTurkcellPremiums(tcellData);
      setExpensesList(expensesData);
      setTurkcellStockList(tcellStockData);
      setDailyClosingsList(closingsData || []);
      setMetrics(dashData.metrics);
      setWeeklyChart(dashData.weeklyChart);
      setMonthlyChart(dashData.monthlyChart);
      setServerIp(dashData.serverIp || 'localhost');
    } catch (err: unknown) {
  ...
  const hasTabPermission = (tabKey) => {
    ...
    return currentUser.permissions && currentUser.permissions.includes(tabKey);
  };
        ...
        {loading ? (
          ...
          <div className="animate-fade-in">
            ...
            {activeTab === 'dashboard' && (
              <div>
                <div className="mb-5">
                  <h2 className="text-xl font-bold tracking-tight text-white mb-0.5">Analiz Paneli</h2>
                  <p className="text-secondary text-xs">Satış verileri ve işletme finansal durum raporu.</p>
                </div>
                ...
                <section className="metrics-grid">
                  <div className="glass-panel metric-card">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white mt-1">{metrics.todaySales.toLocaleString('tr-TR')} TL</h3>
                  ...
                  <div className="glass-panel metric-card">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white mt-1">{metrics.weekSales.toLocaleString('tr-TR')} TL</h3>
                  ...
                  <div className="glass-panel metric-card">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white mt-1">{metrics.monthSales.toLocaleString('tr-TR')} TL</h3>
                  ...
                  <div className="glass-panel metric-card">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white mt-1">{metrics.totalCariReceivables.toLocaleString('tr-TR')} TL</h3>
                  ...
                  <div className="glass-panel metric-card cursor-pointer hover:border-amber-500/20 transition-colors" onClick={() => setActiveTab('turkcell')}>
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white mt-1">{metrics.totalTurkcellProfit.toLocaleString('tr-TR')} TL</h3>
                  ...
                  <div className="glass-panel metric-card">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-red-400 mt-1">{metrics.totalExpenses ? metrics.totalExpenses.toLocaleString('tr-TR') : 0} TL</h3>
                  ...
                  <div className="glass-panel metric-card bg-gradient-to-br from-emerald-950/20 to-emerald-900/10 border border-emerald-500/15">
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-emerald-400 mt-1">{(metrics.totalCihazProfit + metrics.totalTurkcellProfit - metrics.totalExpenses).toLocaleString('tr-TR')} TL</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                        ...
                        {weeklyChart.map((item, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex flex-col items-center flex-1 group">
                              <span className="text-[9px] text-indigo-400 font-bold mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              ></div>
                              <span className="text-[10px] text-secondary mt-2">{item.day}</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {/* Monthly chart */}
                    <div className="glass-panel p-5">
                      <h4 className="font-bold text-white text-sm mb-4">Aylık Satış Analizi (Son 6 Ay)</h4>
                        ...
                        {monthlyChart.map((item, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex flex-col items-center flex-1 group">
                              ...
                              ></div>
                              <span className="text-[10px] text-secondary mt-2">{item.month}</span>
                            </div>
                          );
                        })}
                      </div>
            ...
            {activeTab === 'reports' && (
              <div className="animate-fade-in flex flex-col gap-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold tracking-tight text-white mb-0.5">Satış ve Kârlılık Raporları</h2>
                    <p className="text-secondary text-xs">Ürün bazında satış adetlerini, ciroları ve cihaz grubu net kâr analizini inceleyin.</p>
                  </div>
                ...
                {(() => {
                  ...
                  // Total product sales profit
                  let totalProductProfitForReport = 0;
                  allAggregatedItems.forEach(item => {
                    if (item.id !== 'manual') {
                      totalProductProfitForReport += (item.totalRevenue - (item.purchasePrice * item.quantity));
                    } else {
                      // Manual entry cost is treated as 0
                      totalProductProfitForReport += item.totalRevenue;
                    }
                  });

                  const netPeriodProfit = totalProductProfitForReport - totalExpensesForReport;

                  return (
                    <>
                      {/* Summary Metrics */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 font-sans">
                        <div className="glass-panel p-4 bg-gradient-to-r from-emerald-950/25 to-emerald-900/15 border border-emerald-500/20">
                          <span className="text-[10px] font-semibold text-muted uppercase tracking-wider block text-emerald-400 font-bold">Net Rapor Kârı</span>
                          <h3 className="text-lg font-extrabold text-emerald-400 mt-1">{netPeriodProfit.toLocaleString('tr-TR')} TL</h3>
                          <span className="text-[9px] text-emerald-500/80">Ürün Kârı - Dönem Giderleri</span>
                        </div>
                        <div className="glass-panel p-4 bg-gradient-to-r from-emerald-950/10 to-emerald-900/5 border border-white/5">
                          <span className="text-[10px] font-semibold text-muted uppercase tracking-wider block">Toplam Cihaz Kârı</span>
                          <h3 className="text-lg font-extrabold text-white mt-1">{totalDeviceProfit.toLocaleString('tr-TR')} TL</h3>
                          <span className="text-[9px] text-emerald-400">Satılan cihaz net karı</span>
                        </div>
                        <div className="glass-panel p-4 bg-gradient-to-r from-emerald-950/10 to-emerald-900/5 border border-white/5">
                          <span className="text-[10px] font-semibold text-muted uppercase tracking-wider block">Toplam Aksesuar Kârı</span>
                          <h3 className="text-lg font-extrabold text-white mt-1">{totalOtherProfit.toLocaleString('tr-TR')} TL</h3>
                          <span className="text-[9px] text-emerald-400">Aksesuar & diğer net karı</span>
                        </div>
                        <div className="glass-panel p-4 bg-gradient-to-r from-red-950/20 to-red-900/10 border border-red-500/10">
                          ...
                          <h3 className="text-lg font-extrabold text-red-400 mt-1">-{totalExpensesForReport.toLocaleString('tr-TR')} TL</h3>
                        ...
                        <div className="glass-panel p-4">
                          ...
                          <h3 className="text-lg font-extrabold text-white mt-1">{totalRevenue.toLocaleString('tr-TR')} TL</h3>
                          <span className="text-[9px] text-secondary">Toplam satış cirosu</span>
                        </div>
                      </div>

                      {/* SECTION 1: CİHAZ SATIŞ VE KÂRLILIK RAPORU */}
                      <div className="glass-panel p-0 overflow-hidden border border-emerald-500/10 shadow-lg">
                        <div className="p-4 border-b border-emerald-500/10 bg-emerald-950/5 flex justify-between items-center">
                          ...
                          <span className="text-[10px] text-emerald-400/80 font-semibold font-mono">{deviceReportItems.length} model listelendi</span>
                        </div>
                              ...
                              <table className="w-full text-left text-xs border-collapse">
                                <thead>
                                  <tr className="border-b border-white/5 text-muted font-semibold bg-white/1">
                                    ...
                                    <th className="p-3 text-right w-12">İşlem</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5 font-sans">
                                    ...
                                    return (
                                      <tr key={item.id} className="hover:bg-white/1 transition-colors">
                                        <td className="p-3 font-semibold text-white">
                                          <div>{item.name}</div>
                                          {item.imei && item.imei !== '-' && (
                                            <div className="text-[9px] text-muted font-mono font-bold mt-0.5">IMEI: {item.imei}</div>
                                          )}
                                        </td>
                                        ...
                                        <td className="p-3 text-right font-mono font-bold text-emerald-400">{profit.toLocaleString('tr-TR')} TL</td>
                                        <td className="p-3 text-right">
                                          {item.id !== 'manual' && (
                                            <button
                                              onClick={() => handleOpenEditProductFromReport(item.id)}
                                              title="Ürünü/Maliyeti Düzenle"
                                  ...
                                  {/* Table summary row */}
                                  <tr className="bg-emerald-950/10 font-bold border-t border-emerald-500/20 text-white">
                                    <td className="p-3" colSpan={2}>TOPLAM CİHAZ DEĞERLERİ</td>
                      ...
                      <div className="glass-panel p-0 overflow-hidden border border-white/5 shadow-lg">
                        <div className="p-4 border-b border-white/5 bg-white/2 flex justify-between items-center">
                          <div className="flex items-center gap-2 text-indigo-400">
                            <Package size={16} />
                            <h4 className="font-bold text-sm">Aksesuar & Diğer Ürün Satış Raporu</h4>
                          </div>
                          <span className="text-[10px] text-secondary font-mono">{otherReportItems.length} ürün listelendi</span>
                        </div>
                              ...
                              <table className="w-full text-left text-xs border-collapse">
                                ...
                                <tbody className="divide-y divide-white/5 font-sans">
                                      ...
                                      <tr key={item.id} className="hover:bg-white/1 transition-colors">
                                        <td className="p-3 font-semibold text-white">
                                          ...
                                          {item.barcode && item.barcode !== '-' && (
                                            <div className="text-[9px] text-muted font-mono font-bold mt-0.5">Barkod: {item.barcode}</div>
                                          )}
                                        </td>
                                        <td className="p-3 text-secondary">{item.category}</td>
                                        <td className="p-3 text-right font-mono font-bold text-white">{item.quantity}</td>
                                  ...
                                  <tr className="bg-white/2 font-bold border-t border-white/10 text-white">
                                    <td className="p-3" colSpan={2}>TOPLAM AKSESUAR VE DİĞER</td>
